 ...
 *
 * This is the base repository class for classified entities.
 */
class Classifieds_Entity_Repository_Base_Classified extends EntityRepository
{
    /**
    ...
    public function getDescriptionFieldName()
   {
    ...
    public function getAdditionalTemplateParameters($context = '', $args = array())
    {
        ...
        if ($context == 'controllerAction') {
            ...
            if (in_array($args['action'], array('main', 'view'))) {
                ...
                $listHelper = new Classifieds_Util_ListEntries(ServiceUtil::getManager());
                $templateParameters['workflowStateItems'] = $listHelper->getEntries('classified', 'workflowState');
                $templateParameters['kindItems'] = $listHelper->getEntries('classified', 'kind');
                $booleanSelectorItems = array(
                    array('value' => 'no', 'text' => __('No')),
                    array('value' => 'yes', 'text' => __('Yes'))
                );
                $templateParameters['termsItems'] = $booleanSelectorItems;
            }
    
            // initialise Imagine preset instances
            $imageHelper = new Classifieds_Util_Image(ServiceUtil::getManager());
    ...
    protected function getViewQuickNavParameters($context = '', $args = array())
    {
        ...
        $parameters = array();
        $parameters['catIdList'] = ModUtil::apiFunc('Classifieds', 'category', 'retrieveCategoriesFromRequest', array('ot' => 'classified', 'source' => 'GET'));
        ...
        $parameters['kind'] = isset($this->controllerArguments['kind']) ? $this->controllerArguments['kind'] : FormUtil::getPassedValue('kind', '', 'GET');
        ...
        $parameters['terms'] = isset($this->controllerArguments['terms']) ? $this->controllerArguments['terms'] : FormUtil::getPassedValue('terms', '', 'GET');
    ...
    public function truncateTable()
    {
        ...
        $qb->delete('Classifieds_Entity_Classified', 'tbl');
        $query = $qb->getQuery();
    ...
    public function updateCreator($userId, $newUserId)
    {
        ...
        $qb = $this->getEntityManager()->createQueryBuilder();
        $qb->update('Classifieds_Entity_Classified', 'tbl')
     ...
     *
     * @return array|Classifieds_Entity_Classified retrieved data array or Classifieds_Entity_Classified instance
     *
     * @throws InvalidArgumentException Thrown if invalid parameters are received
     */
     ...
     * @param boolean $slimMode If activated only some basic fields are selected without using any joins (optional) (default=false).
     *
     * @return ArrayCollection collection containing retrieved Classifieds_Entity_Classified instances
     */
    ...
    public function addCommonViewFilters(QueryBuilder $qb)
    {
        ...
        foreach ($parameters as $k => $v) {
            if ($k == 'catId') {
                 ...
                 */
                $qb = ModUtil::apiFunc('Classifieds', 'category', 'buildFilterClauses', array('qb' => $qb, 'ot' => 'classified', 'catids' => $v));
            } elseif ($k == 'searchterm') {
                // quick search
                if (!empty($v)) {
                    $qb = $this->addSearchFilter($qb, $v);
                }
            } elseif (in_array($k, array('terms'))) {
                // boolean filter
                if ($v == 'no') {
                    $qb->andWhere('tbl.' . $k . ' = 0');
                } elseif ($v == 'yes' || $v == '1') {
    ...
    protected function applyDefaultFilters(QueryBuilder $qb, $parameters = array())
    {
        ...
        if ($currentType == 'admin' && $currentModule == 'Classifieds') {
            return $qb;
        }
    
        if (!in_array('workflowState', array_keys($parameters)) || empty($parameters['workflowState'])) {
            // per default we show approved classifieds only
            $onlineStates = array('approved');
            $showOnlyOwnEntries = (int) FormUtil::getPassedValue('own', ModUtil::getVar('Classifieds', 'showOnlyOwnEntries', 0), 'GETPOST');
            if ($showOnlyOwnEntries == 1) {
                // allow the owner to see his deferred classifieds
    ...
    protected function getCountQuery($where = '', $useJoins = true)
    {
        ...
        $selection = 'COUNT(tbl.id) AS numClassifieds';
        if ($useJoins === true) {
        ...
        $qb = $this->getEntityManager()->createQueryBuilder();
        $qb->select($selection)
           ->from('Classifieds_Entity_Classified', 'tbl');
     ...
     * @param int    $excludeId  Id of classifieds to exclude (optional).
     *
    ...
    public function archiveObjects()
    {
        ...
        foreach ($affectedEntities as $entity) {
            ...
            ModUtil::apiFunc('Classifieds', 'cache', 'clearItemCache', $cacheArgs);
        }
