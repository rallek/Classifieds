 ...
 */
class Classifieds_Block_Base_Navi extends Zikula_Controller_AbstractBlock
{
    /**
    ...
    {
        SecurityUtil::registerPermissionSchema('Classifieds:NaviBlock:', 'Block title::');
    ...
    public function info()
    {
        ...
        if (!ModUtil::available('Classifieds')) {
            $requirementMessage .= $this->__('Notice: This block will not be displayed until you activate the Classifieds module.');
        }
    
        return array('module'          => 'Classifieds',
                     'text_type'       => $this->__('Classifieds navigator'),
                     'text_type_long'  => $this->__('Display navigator for Classifieds objects.'),
                     'allow_multiple'  => true,
                     'form_content'    => false,
                     'form_refresh'    => false,
                     'show_preview'    => true,
    ...
    public function display($blockinfo)
    {
        ...
        if (!SecurityUtil::checkPermission('Classifieds:NaviBlock:', "$blockinfo[title]::", ACCESS_OVERVIEW)) {
            return false;
        }
        ...
        if (!isset($vars['template'])) {
            $vars['template'] = 'navi_' . DataUtil::formatForOS($vars['objectType']) . '_display.tpl';
        }
        ...
        if (!isset($vars['catIds'])) {
            $primaryRegistry = ModUtil::apiFunc('Classifieds', 'category', 'getPrimaryProperty', array('ot' => $vars['objectType']));
        ...
        if (in_array($vars['objectType'], $this->categorisableObjectTypes)) {
            $properties = ModUtil::apiFunc('Classifieds', 'category', 'getAllProperties', array('ot' => $objectType));
        }
    
        // apply category filters
        if (in_array($objectType, $this->categorisableObjectTypes)) {
            if (is_array($vars['catIds']) && count($vars['catIds']) > 0) {
                $qb = ModUtil::apiFunc('Classifieds', 'category', 'buildFilterClauses', array('qb' => $qb, 'ot' => $objectType, 'catids' => $vars['catIds']));
            }
        }
        ...
        if (empty($blockinfo['title'])) {
            $blockinfo['title'] = $this->__('Classifieds items');
        }
    ...
    protected function getDisplayTemplate($vars)
    {
        ...
        $templateForObjectType = str_replace('navi_', 'navi_' . DataUtil::formatForOS($vars['objectType']) . '_', $templateFile);
    ...
    protected function getSortParam($vars, $repository)
    {
        ...
        if ($vars['sorting'] == 'newest') {
            $idFields = ModUtil::apiFunc('Classifieds', 'selection', 'getIdFields', array('ot' => $vars['objectType']));
            if (count($idFields) == 1) {
    ...
    public function modify($blockinfo)
    {
        ...
        $this->view->clear_cache('block/navi_display.tpl');
        $this->view->clear_cache('block/navi_' . DataUtil::formatForOS($vars['objectType']) . '_display.tpl');
        $this->view->clear_cache('block/navi_display_description.tpl');
        $this->view->clear_cache('block/navi_' . DataUtil::formatForOS($vars['objectType']) . '_display_description.tpl');
    
        // Return the output that has been generated by this function
        return $this->view->fetch('block/navi_modify.tpl');
    }
    ...
    public function update($blockinfo)
    {
        ...
        $vars['objectType'] = $this->request->request->filter('objecttype', 'classified', FILTER_SANITIZE_STRING);
        ...
        $vars['catIds'] = array($primaryRegistry => array());
        if (in_array($vars['objectType'], $this->categorisableObjectTypes)) {
            $vars['catIds'] = ModUtil::apiFunc('Classifieds', 'category', 'retrieveCategoriesFromRequest', array('ot' => $vars['objectType']));
        }
        ...
        // clear the block cache
        $this->view->clear_cache('block/navi_display.tpl');
        $this->view->clear_cache('block/navi_' . ucwords($vars['objectType']) . '_display.tpl');
        $this->view->clear_cache('block/navi_display_description.tpl');
        $this->view->clear_cache('block/navi_' . ucwords($vars['objectType']) . '_display_description.tpl');
